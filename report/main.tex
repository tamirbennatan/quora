...
We also think that an interesting extension to this work would be to extract features that concern the grammatical functions of the words in each question. For example, one could construct a dependency parse tree for each question, and compare the subjects, direct objects and thus extract headwords as a result. We briefly experimented with named entity recognition (NER), but it was quickly dropped in the early process upon realizing that pre-trained NER models (such as the one SpaCy provides) do very poorly on specialized datasets like Quora's. Manually trained, task-specific NER models could however provide meaningful information to the question duplication problem.
